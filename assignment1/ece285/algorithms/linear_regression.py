...
class Linear(object):
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, weights: np.ndarray) -> np.ndarray:
        ...
        for i in(range(self.epochs)): 
            ...
            for img,label in (zip(X_train, y_train)): 
                y_pred = -np.ones(self.n_class)
                y_pred[label] = 1
                grad += np.outer(2 * (self.w @ img - y_pred), img)
            grad /= N 
            grad += 2 * self.weight_decay * self.w
            self.w -= self.lr * grad
        return self.w
    ...
    def predict(self, X_test: np.ndarray) -> np.ndarray:
        ...
        for i in range(N_test): 
            score = self.w @ X_test[i]
            predicted_labels[i] = np.argmax(score)
